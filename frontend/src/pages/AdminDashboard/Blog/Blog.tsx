...
const Blog = () => {
  ...
  const { data: blogs, loading: blogsLoding } = useFetch<BlogType[]>({
    apiCall: () => getBlogsApi()
  });

  console.log(blogs)

  const { data: recentBlogs, loading: recentBlogsLoding, } = useFetch<BlogType[]>({
  ...
  return (
    <div className='sm:p-[1rem] pb-[2rem] '>
      ...
      {
        currentBlogs && currentBlogs.length > 0 && !blogsLoding ? <>
          <div className='w-full bg-white mt-[2rem] box-shadow sm:rounded-lg  p-[1rem]'>
